...
HttpServer::HttpServer(std::string &serverName, InetAddr &localAddr, std::string &homeDir, int numThreads, double delayCLoseSec)
    : m_name(serverName),
...
void HttpServer::start() {
    EventLoop baseLoop;
    TcpServer tcpServer(&baseLoop, m_name, 4, m_localAddr, m_delayCloseSec);
